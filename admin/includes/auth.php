...
function requireAuth(): void {
  if (empty($_SESSION['fu_admin'])) {
    header('Location: /prihlaseni');
    exit;
  }
  if (!empty($_SESSION['fu_last_activity']) && (time() - $_SESSION['fu_last_activity']) > SESSION_IDLE_LIMIT) {
    logout();
    header('Location: /prihlaseni?timeout=1');
    exit;
  }
  $_SESSION['fu_last_activity'] = time();
}

function isLoggedIn(): bool {
  return !empty($_SESSION['fu_admin']);
}
...
function registerFailedLogin(): void {
  ...
  fuLogSecurity("Failed login attempt from $ip (attempt {$all[$ip]['count']})");
}
...
function fuLogSecurity(string $msg): void {
  $line = '[' . date('Y-m-d H:i:s') . '] ' . $msg . "\n";
  @file_put_contents(__DIR__ . '/../../data/.security.log', $line, FILE_APPEND | LOCK_EX);
}

function readJson(string $file): array { return fuReadJson($file); }
function writeJson(string $file, array $data): bool { return fuWriteJson($file, $data); }
